// Ex 6 - Attaching objects to the robot, Panda
...
double limit_fcn(double input, double min, double max)
{
  if(input>max)
    return max;
  else if(input<min)
...
void moveGripper(double dist)
{
  ...
  moveit::planning_interface::MoveGroupInterface move_group(move_group_node, PLANNING_GROUP_GRIPPER);

  moveit::planning_interface::MoveGroupInterface::Plan my_plan;

  move_group.setJointValueTarget("panda_finger_joint1", limit_fcn(dist, 0, 0.04));
  move_group.setJointValueTarget("panda_finger_joint2", limit_fcn(dist, 0, 0.04));

  bool success = (move_group.plan(my_plan) == moveit::core::MoveItErrorCode::SUCCESS);

  RCLCPP_INFO(LOGGER, "Gripping moves the jaws %s", success ? "" : "FAILED");
...
int main(int argc, char** argv)
{
  ...
  visual_tools.trigger();
  visual_tools.prompt("Press 'next' in the RvizVisualToolsGui window to open the gripper");


  // Opening the gripper
  // ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  moveGripper(0.04);
  visual_tools.publishText(text_pose, "Openning_Gripper", rvt::WHITE, rvt::XLARGE);
  visual_tools.trigger();
  ...
  std::vector<double> joint_group_positions =
     {80.0*D2R, -15.0*D2R, 10.0*D2R, -110.0*D2R, 0.0*D2R, 90.0*D2R, 0.0*D2R}; //Unit: radian
